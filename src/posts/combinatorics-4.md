---
title: "Combinatorics 4: Counting & Bijections"
date: "2020-03-15"
tags: ["Math", "Combinatorics", "Olympiad", "Problem Solving"]
excerpt: "A comprehensive guide to combinatorial principles, from addition/multiplication rules to circular permutations, PIE, and bijective proofs."
---

## Two basic facts

### Fact (Addition principle)
...
### Fact (Multiplication principle)
Let $A_1, A_2,\cdots A_k$ be events with $n_1, n_2, ... n_k$ possible outcomes, respectively. Then the total number of outcomes for the sequence of these k events is $n_1 \times n_2 \times ... \times n_k$.

I am also providing some exercises under this segment for completion.

**Exercise:** A college offers 7 courses in the morning and 5 in the evening. Find the possible number of choices with the student if he wants to study one course in the morning and one in the evening.

**Exercise:** A person wants to go from station A to station C via station B. There are three routes from A to B and four routes from B to C. In how many ways can he travel from A to C?

**Exercise (JEE Mains 2002):** Find the total number of 4 digit numbers with the digits $0,1,2,3,5,7$ where repetition is allowed.

**Exercise (CMI 2011 UG entrance subjective):** Let S be the set of all 5-digit numbers that contain the digits 1,3,5,7 and 9 exactly once (in usual base 10 representation). Show that the sum of all elements of S is divisible by 11111. Find this sum.

**Exercise (JEE Mains 2012):** The number of ways in which three identical balls, but of different colour could be selected from a group of 10 white, 9 green and 7 black balls is?

**Exercise (CMI 2011 UG entrance objective):** The word MATHEMATICS consists of 11 letters. The number of distinct ways to rearrange these letters is?

**Exercise (JEE Mains 2004):** How many ways are there to arrange the letters of the word $GARDEN$ in alphabetical order?

**Exercise:** There are 8 boys and 7 girls in a group. For each of the tasks specified below, write an expression for the number of ways of doing it.
*   Sitting in a row so that all boys sit contiguously and all girls sit contiguously, i.e., no girl sits between any two boys and no boy sits between any two girls
*   Sitting in a row so that between any two boys there is a girl and between any two girls there is a boy
*   Choosing a team of six people from the group.
*   Choosing a team of six people consisting of unequal number of boys and girls

**Remark (More techniques):** There are some techniques which are worth noting in this section like Principle of Inclusion exclusion, Complementary and constructive counting, fractional steps and classification and possibly even counting in two ways. Some of the exercise problems also use these techniques for an elegant solution. Try to append more of these in your repertoire.
...
We denote the number of ways of combining of choosing $n$ objects from $m$ where obviously $m \ge n$ is denoted by ${m}\choose {n}$

Once we have the number of combinations, we would easily have the number of permutations because $^nP_m={n\choose m}\cdot m!$

### Problem
Let $S$ be the set of all numbers which could be made using the digits $1,3,5,7$, then find out
*   Cardinality of $S$
...
**Exercise:** Generalise the above problem.

### Problem
Find the number of diagonals of a regular $n$-gon.

**Question:** What happens when it is an irregular $n$-gon?

### Circular permutations
So evidently we are going to have the circular permutations to be $(n-1)!$ in here because we are to consider the shift operation as a group. I'll elaborate what I mean in the following proposition.

**Definition (Shift operation):** We are to shift the places of each permutation of a particular set or an object. for example, in case of the permutation of $ABCD$, our shift operation would yield the result $ABCD \mapsto DABC$.

**Proposition:** Consider that we have a particular permutation of $ABCD$ around the circle. Then obviously we notice that a shift over all the elements of the permutations are just the copies of itself, just rotated by a unit. By our claim we can prove that there are $n$ such groups. Hence our total permutations are going to be $\frac{n!}{n}=(n-1)!$.

**Exercise:** If 16 persons are sitting in a circle . Find out the number of ways of selecting 5 persons such that no two of them are consecutive.

**Exercise:** Find the number of ways in which 6 persons $A,B,C,D,E,F$ can be seated in a ring so that $A$ sits between $B$ and $C$.

### Theorem (Principle of Inclusion and Exclusion)
Let $|\cdot |$ denote the cardinality or the number of elements in set $(\cdot )$ then the principle of inclusion exclusion states that,
$$ |A\cup B|=|A|+|B|-|A\cap B| $$
for two joint sets $A,B$.
...
**Question:** Can we generalize PIE for $n$ sets?

**Exercise:** Try constructing the picture for PIE in four sets.

### Problems for this section

**Problem (AIME 2011):** In a town of $351$ adults, every adult owns a car, motorcycle, or both. If $331$ adults own cars and $45$ adults own motorcycles, how many of the car owners do not own a motorcycle?
...
**Exercise:** Do the algebraic proof.
...
*Proof:*
This is just a special case of the previous theorem, just put $k=1$ and we solve it.
...
**Example:** Let in an auditorium we have 200 seats, provided we cannot have two students cannot sit in the same seat. So there are $\le 200$ students because we do not necessarily have to have all the seats full, but at least some of them. Its called to have "every element in $A$ has a pre-image in $B$".
...
*Solution:*
We state that there is a bijection between the total number of paths, and the number of ways or arranging $4Rs$ and $3Us$ which is $7\choose 3$.
